...
{
    auto cmp = [](pair<pair<int, int>, vector<vector<int>>> &a, pair<pair<int, int>, vector<vector<int>>> &b)
    {
        return a.first.first > b.first.first;
    };
    priority_queue<pair<pair<int, int>, vector<vector<int>>>, vector<pair<pair<int, int>, vector<vector<int>>>>, decltype(cmp)> pq(cmp);
    ...
    while (!pq.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (newX >= 0 && newX < curr.size() && newY >= 0 && newY < curr.size())
            {
                ...
                pq.push({{(curr_d + curr_h), curr_d + 1}, curr});
                swap(curr[x][y], curr[newX][newY]);
            }
        }
    }
